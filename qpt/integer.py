...
def to_rect_coords(pan, tilt):
    """Convert the spherical coords to rectangular coords.
    Assuming the magnituded of the vector in spherical coords is 1.
        ie. (1, theta, phi)

    return: list representing the rect. coords [x,y,z]
    """
    rect_coords = []
    x = sin(pan*pi/180)*cos(tilt*pi/180)
    y = sin(pan*pi/180)*sin(tilt*pi/180)
    z = cos(pan*pi/180)
    rect_coords.append(x)
    rect_coords.append(y)
    rect_coords.append(z)
    return rect_coords
